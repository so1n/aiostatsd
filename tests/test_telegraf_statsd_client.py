...
@pytest.fixture
async def telegraf_statsd_client():
...
class TestTelegrafStatsdClient:
    async def test_set(self, telegraf_statsd_client: aio_statsd.TelegrafStatsdClient, udp_server):
        telegraf_statsd_client.set("test.key", 1)
        assert await udp_server.get() == b"test.key:1|s"

    async def test_counter(self, telegraf_statsd_client: aio_statsd.TelegrafStatsdClient, udp_server):
    ...
    async def test_gauge(self, telegraf_statsd_client: aio_statsd.TelegrafStatsdClient, udp_server):
