...
class Connection(object):
    def __init__(
        ...
        self._debug = debug
        self._protocol_flag = protocol_flag
        self._close_timeout = close_timeout
        self._loop = loop if loop else asyncio.get_event_loop()
        if protocol_flag == Protocol.Udp:
            self._protocol: DatagramProtocol = DatagramProtocol()
        elif protocol_flag == Protocol.Tcp:
            self._protocol: TcpProtocol = TcpProtocol()
        else:
            raise RuntimeError
    ...
    async def connect(self) -> NoReturn:
        if self._protocol_flag == Protocol.Udp:
            await self._loop.create_datagram_endpoint(
                lambda: self._protocol,
                remote_addr=(self._host, self._port)
            )
        elif self._protocol_flag == Protocol.Tcp:
            await self._loop.create_connection(
                lambda: self._protocol,
                host=self._host, port=self._port
            )
        logging.debug(f'create {self._protocol_flag}:{self._host}:{self._port} connection')
    ...
    async def _close(self) -> NoReturn:
        ...
        logging.debug(f'close {self._protocol_flag}:{self._host}:{self._port} connection')
